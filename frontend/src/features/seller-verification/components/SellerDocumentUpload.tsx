...
type Props = {
  ...
  deletingId: string | null; accessingId: string | null; uploadError: unknown; deleteError: unknown;
...
function UploadCard({ type, title, hint, document, busy, deletingId, accessingId, onUpload, onDelete, onAccess }: UploadCardProps) {
  ...
  return <section className="rounded-lg border border-border bg-white p-4" aria-labelledby={`upload-${type}-title`}>
    ...
    {document ? <div className="mt-4 rounded-md bg-surface p-3">{previewUrl ? <img src={previewUrl} alt={`${title} đã tải lên`} className="mb-3 aspect-[4/3] w-full rounded-md border border-border bg-white object-contain"/> : null}<div className="flex min-w-0 items-center gap-2"><FileText size={18} className="shrink-0 text-primary-700"/><p className="truncate text-sm font-medium">{document.originalFileName}</p></div><div className="mt-3 flex flex-wrap gap-2"><Button type="button" variant="secondary" disabled={busy || accessingId === document.id} onClick={() => onAccess(document.id)}>{accessingId === document.id ? 'Đang mở...' : 'Xem ảnh gốc'}</Button><Button type="button" variant="danger" disabled={busy || deletingId === document.id} onClick={() => onDelete(document.id)}><Trash2 size={16}/>{deletingId === document.id ? 'Đang xóa...' : 'Xóa'}</Button></div></div> : <>
      {fileError ? <Alert tone="danger" className="mt-3">{fileError}</Alert> : null}
      <input ref={inputRef} id={`seller-file-${type}`} type="file" accept="image/jpeg,image/png,.jpg,.jpeg,.png" disabled={busy} className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; setFileError(null); if (file) { if (file.size <= 0) setFileError('Tệp đã chọn không có nội dung. Vui lòng chọn ảnh khác.'); else if (file.size > 10 * 1024 * 1024) setFileError('Ảnh vượt quá 10 MB. Vui lòng giảm dung lượng hoặc chọn ảnh khác.'); else if (!['image/jpeg', 'image/png'].includes(file.type)) setFileError('Định dạng không hợp lệ. Vui lòng chọn ảnh JPG hoặc PNG.'); else onUpload(type, file); } event.currentTarget.value = ''; }}/>
      <button type="button" disabled={busy} onClick={() => inputRef.current?.click()} className="mt-4 flex min-h-32 w-full flex-col items-center justify-center rounded-lg border border-dashed border-border bg-surface px-4 text-center transition hover:border-primary-600 hover:bg-primary-50 focus:outline-none focus:ring-2 focus:ring-primary-600 disabled:cursor-not-allowed disabled:opacity-60"><UploadCloud size={25} className="text-primary-700" aria-hidden="true"/><span className="mt-2 font-medium text-ink">{busy ? 'Đang tải lên...' : 'Tải lên'}</span><span className="mt-1 text-xs text-muted">JPG hoặc PNG, tối đa 10 MB</span></button>
    </>}
  </section>;
}
...
export function SellerDocumentUpload({ sellerType, businessType, documents, isUploading, deletingId, accessingId, uploadError, deleteError, onUpload, onDelete, onAccess }: Props) {
  ...
  return <div className="space-y-6">
    {uploadError ? <Alert tone="danger">Không thể tải ảnh: {getErrorMessage(uploadError)}</Alert> : null}
    {deleteError ? <Alert tone="danger">Không thể xóa ảnh: {getErrorMessage(deleteError)}</Alert> : null}
    ...
    {sellerType === 'Business' ? <section aria-labelledby="registration-upload-title"><h2 id="registration-upload-title" className="text-lg font-semibold">Giấy chứng nhận đăng ký <span className="text-danger" aria-hidden="true">*</span></h2><p className="mt-1 text-sm text-muted">Ảnh đầu tiên là bắt buộc. Có thể tải thêm tối đa 2 ảnh bổ sung nếu giấy chứng nhận có nhiều trang.</p><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{registrations.map((document, index) => <UploadCard key={document.id} type="BusinessRegistration" title={index === 0 ? 'Ảnh giấy chứng nhận đăng ký (bắt buộc)' : `Ảnh bổ sung ${index + 1} (không bắt buộc)`} hint={labels.BusinessRegistration} document={document} busy={isUploading} deletingId={deletingId} accessingId={accessingId} onUpload={onUpload} onDelete={onDelete} onAccess={onAccess}/>)}{registrations.length < 3 ? <UploadCard type="BusinessRegistration" title={registrations.length === 0 ? 'Tải ảnh giấy chứng nhận đăng ký (bắt buộc)' : 'Thêm ảnh bổ sung (không bắt buộc)'} hint={registrations.length === 0 ? 'Phải tải ít nhất 1 ảnh rõ nét để tiếp tục' : 'Chỉ cần khi giấy chứng nhận có nhiều trang'} busy={isUploading} deletingId={deletingId} accessingId={accessingId} onUpload={onUpload} onDelete={onDelete} onAccess={onAccess}/> : null}</div></section> : null}
    {sellerType === 'Individual' || businessType === 'HouseholdBusiness' ? <section className="rounded-lg border border-border bg-surface p-5" aria-labelledby="face-title"><div className="flex gap-3"><ScanFace className="shrink-0 text-primary-700"/><div><h2 id="face-title" className="font-semibold">Quét khuôn mặt realtime</h2><p className="mt-1 text-sm text-muted">Bắt buộc mở camera và chụp trực tiếp để lưu vào hồ sơ đối chiếu.</p></div></div><FaceCapture document={find('FaceVerification')} busy={isUploading} deletingId={deletingId} accessingId={accessingId} onUpload={onUpload} onDelete={onDelete} onAccess={onAccess}/></section> : null}
  </div>;
}
