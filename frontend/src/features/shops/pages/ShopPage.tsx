...
export function ShopPage() {
  ...
  return <div className="space-y-6">
    ...
    <nav aria-label="Danh mục của gian hàng" className="flex gap-2 overflow-x-auto rounded-lg border border-border bg-white p-2 shadow-panel"><button type="button" onClick={() => updateParams({ search })} className={`min-h-11 shrink-0 rounded-md px-4 text-sm font-medium ${!category ? 'bg-primary-600 text-white' : 'text-ink hover:bg-surface'}`}>Tất cả sản phẩm</button>{categories.map((item) => <button key={item.id} type="button" onClick={() => updateParams({ category: item.slug, search })} className={`flex min-h-11 shrink-0 items-center gap-2 rounded-md px-3 text-sm font-medium ${category === item.slug ? 'bg-primary-600 text-white' : 'text-ink hover:bg-surface'}`}>{item.imageUrl ? <img src={item.imageUrl} alt="" className="h-9 w-9 rounded-md object-cover" loading="lazy"/> : null}<span>{item.categoryName} <span className="opacity-70">({item.productCount})</span></span></button>)}</nav>
