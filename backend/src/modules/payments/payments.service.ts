...
export class PaymentsService {
  ...
  async createVnpayPaymentUrl(
    ...
    if (payment.paymentStatus !== PAYMENT_STATUS_PENDING) {
      throw new BadRequestException({
        ...
        message: 'Giao dịch này không còn chờ thanh toán.',
      });
    }

    const createdAt = new Date();
    const expiresAt = new Date(
      createdAt.getTime() + this.vnpay.getPaymentTtlMinutes() * 60_000,
    );
    ...
    const paymentUrl = this.vnpay.createPaymentUrl({
      ...
      clientIp,
      transactionRef,
      orderInfo: `Thanh toan ${payment.paymentCode}`,
      createdAt,
      expiresAt,
    });
    await this.prisma.$transaction([
      this.prisma.payment.update({
        where: { id },
        data: { expiredAt: expiresAt, updatedAt: createdAt },
      }),
    ...
    await this.prisma.$transaction(async (tx) => {
      const claimed = await tx.paymentTransaction.updateMany({
        ...
        data: {
          ...
          providerResponseMessage: isSuccess
      ...
      await tx.order.update({
        ...
        data: { paymentStatus: nextPaymentStatus, updatedAt: now },
      });
      await tx.paymentStatusHistory.create({
        data: {
          paymentId: transaction.paymentId,
          fromStatus: transaction.payment.paymentStatus,
          toStatus: nextPaymentStatus,
          reason: `VNPay response ${params.vnp_ResponseCode}`,
        },
      });
    });
    return {
      ...
      orderId: transaction.payment.orderId.toString(),
      paymentStatus: nextPaymentStatus,
      message: isSuccess
        ? 'Thanh toán thành công.'
        : 'Thanh toán không thành công.',
  ...
  async markFakeSuccess(user: AuthenticatedUser, paymentId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.order.update({
        ...
        data: { paymentStatus: PAYMENT_STATUS_PAID, updatedAt: now },
      });
      await tx.paymentStatusHistory.create({
        data: {
          paymentId: id,
      ...
      return {
        ...
        idString: updated.id.toString(),
        paymentCode: updated.paymentCode,
        paymentMethod: {
          id: updated.paymentMethod.id.toString(),
          idString: updated.paymentMethod.id.toString(),
          methodCode: updated.paymentMethod.methodCode,
          methodName: updated.paymentMethod.methodName,
          isOnline: updated.paymentMethod.isOnline,
        },
  ...
  private callbackResult(
    ...
    return {
      ...
      message: success ? 'Thanh toán thành công.' : 'Giao dịch đã được xử lý.',
    };
  }
