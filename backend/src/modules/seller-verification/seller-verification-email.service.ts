...
export class SellerVerificationEmailService {
  ...
  async sendRevisionRequested(email: string, name: string, shopName: string, reason: string) { await this.sendNotification(email, 'Hồ sơ người bán cần bổ sung', `<h2>Hồ sơ cần được bổ sung</h2><p>Xin chào ${this.escape(name)},</p><p>Hồ sơ cửa hàng <strong>${this.escape(shopName)}</strong> cần cập nhật trước khi xét duyệt tiếp.</p><p><strong>Nội dung cần bổ sung:</strong></p><p>${this.escape(reason)}</p><p>Vui lòng đăng nhập Kênh người bán, chỉnh sửa hồ sơ và gửi lại.</p>`); }
  async sendRejected(email: string, name: string, shopName: string, reason: string) { await this.sendNotification(email, 'Hồ sơ người bán chưa được chấp thuận', `<h2>Hồ sơ chưa được chấp thuận</h2><p>Xin chào ${this.escape(name)},</p><p>Hồ sơ cửa hàng <strong>${this.escape(shopName)}</strong> chưa được chấp thuận.</p><p><strong>Lý do:</strong></p><p>${this.escape(reason)}</p><p>Vui lòng đăng nhập Kênh người bán để xem phản hồi và cập nhật hồ sơ phù hợp.</p>`); }
